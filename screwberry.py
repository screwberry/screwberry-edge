...
if __name__ == '__main__':
    try:
        client = IoTHubDeviceClient.create_from_connection_string(cfg.CONNECTION)
    except ValueError as e:
        exception_factory("Connection to IoT Hub failed", e)
    else:
        try:
            print ("Connection to IoT Hub established (Ctrl-C to exit)")
            RuuviTagSensor.get_datas(handle_data)
        except AttributeError as e:
            exception_factory("Error in configuration", e)
    finally:
        print ("Exiting...")
